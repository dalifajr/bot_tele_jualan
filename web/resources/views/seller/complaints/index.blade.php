...
@section('title', __('Kelola Komplain Pelanggan'))
@section('page_subtitle', __('Komplain'))

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold mb-1">{{ __('Kelola Komplain Pelanggan') }}</h4>
        <p class="text-muted mb-0">{{ __('Daftar keluhan dari pembeli produk digital Anda') }}</p>
    </div>
</div>
...
<div class="card border-0 shadow-sm overflow-hidden" style="border-radius: 16px;">
    <div class="card-body p-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-end px-4">
                            <a href="{{ route('seller.complaints.show', $complaint->id) }}" class="btn btn-sm btn-light text-primary rounded-circle" title="{{ __('Lihat Detail & Resolusi') }}">
        ...
        <div class="text-center py-5">
            ...
            <p class="text-muted mb-0">{{ __('Belum ada komplain pelanggan saat ini.') }}</p>
        </div>
        @endif
    </div>
</div>
