...
@section('title', __('Backup & Restore - Progress Pemulihan'))
@section('page_subtitle', __('Backup & Restore'))

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h4 class="fw-bold mb-1">{{ __('Backup & Restore') }}</h4>
        <p class="text-muted mb-0">{{ __('Pemulihan data cadangan secara real-time') }}</p>
    </div>
</div>
...
<div class="card border-0 shadow-sm" style="border-radius: 16px; background-color: var(--bs-card-bg);">
    <div class="card-body p-4">
        <div class="d-flex align-items-center justify-content-between mb-3">
            <div>
                <h5 class="fw-bold mb-1">
                    ...
                    {{ __('Status:') }} <span id="statusText" class="text-danger">{{ __('Sedang memproses pemulihan data...') }}</span>
                </h5>
                <p class="text-muted small mb-0">
                    {{ __('Berkas:') }} <strong class="text-body">{{ $filename }}</strong> {{ __('&bull; Mode:') }} <strong class="text-body">{{ $mode === 'overwrite' ? 'Full Overwrite' : 'Smart Merge' }}</strong>
                </p>
            </div>
        ...
        <div id="logConsole" class="bg-dark text-light p-3 font-monospace mb-4" style="height: 350px; overflow-y: auto; border-radius: 12px; font-size: 0.85rem; line-height: 1.5; border: 1px solid #343a40;">
            <div class="text-secondary">{{ __('[System] Memulai streaming pemulihan data...') }}</div>
        </div>

        <div class="d-flex justify-content-end gap-2">
            <a href="{{ route('admin.backup.index') }}" id="btnDone" class="btn btn-secondary rounded-pill px-4 py-2 fw-bold d-none">
                <i class="fas fa-arrow-left me-2"></i>{{ __('Kembali ke Dashboard') }}
            </a>
        </div>
    </div>
</div>
