...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-muted mb-0">Verifikasi dan proses penarikan dana dari dompet saldo para seller mitra</p>
    </div>
</div>
...
<div class="card border-0 shadow-sm overflow-hidden" style="border-radius: 16px;">
    <div class="card-body p-0">
            ...
            <table class="table table-hover align-middle mb-0">
                <thead>
                    <tr class="text-secondary small border-bottom">
                        ...
                        <th class="py-3 border-0">Tujuan Rekening / Bank</th>
                        <th class="py-3 border-0">Jumlah Penarikan</th>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($withdrawal->status === 'pending')
                                <span class="badge bg-warning-subtle text-warning rounded-pill px-3 py-1"><i class="fas fa-spinner fa-spin me-1"></i>Menunggu</span>
                            @elseif($withdrawal->status === 'approved')
                                <span class="badge bg-success-subtle text-success rounded-pill px-3 py-1"><i class="fas fa-check-circle me-1"></i>Selesai</span>
                            @else
                                <span class="badge bg-danger-subtle text-danger rounded-pill px-3 py-1"><i class="fas fa-times-circle me-1"></i>Ditolak</span>
                            @endif
                        </td>
                            ...
                            @else
                                ...
                                @else
                                    <button class="btn btn-sm btn-light rounded-pill px-3" data-bs-toggle="modal" data-bs-target="#viewRejectionModal{{ $withdrawal->id }}">
                                        <i class="fas fa-comment-slash me-1"></i> Alasan Ditolak
                                    </button>
                                @endif
                            @endif
                        </td>
        ...
        <div class="text-center py-5">
            ...
            <p class="text-muted mb-0">Tidak ada pengajuan penarikan dana saat ini.</p>
        </div>
        @endif
    </div>
</div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 16px; border: none;">
            <div class="modal-header border-0 pb-0">
                <h5 class="fw-bold">Persetujuan & Unggah Bukti Payout</h5>
            ...
            <form action="{{ route('admin.withdrawals.approve', $withdrawal->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body p-4">
                    <div class="alert alert-info border-0 rounded-4 small py-2 mb-3">
                        <i class="fas fa-info-circle me-1"></i> Silakan lakukan transfer manual terlebih dahulu sesuai dengan rekening tujuan di bawah ini sebesar nominal penarikan, kemudian unggah struk bukti transfer.
                    </div>
                    <div class="mb-3 bg-light p-3 rounded-4">
                        <p class="mb-1 text-muted small">Tujuan Rekening</p>
                        <h6 class="fw-bold mb-2 text-primary">{{ strtoupper($withdrawal->bank_name) }} — {{ $withdrawal->account_number }}</h6>
                        <p class="mb-1 text-muted small">Nama Pemilik Rekening</p>
                        <h6 class="fw-bold mb-2">{{ $withdrawal->account_holder }}</h6>
                        <p class="mb-1 text-muted small">Jumlah Transfer</p>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-text">Maksimal ukuran file 2MB dengan tipe JPEG, PNG, JPG.</div>
                    </div>
                </div>
                <div class="modal-footer border-0 pt-0">
                    <button type="button" class="btn btn-light rounded-pill px-4" data-bs-dismiss="modal">Batal</button>
                    <button type="submit" class="btn btn-primary rounded-pill px-4">Setujui & Selesaikan</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="rejectModal{{ $withdrawal->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 16px; border: none;">
            <div class="modal-header border-0 pb-0">
                <h5 class="fw-bold text-danger">Tolak Pengajuan Penarikan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('admin.withdrawals.reject', $withdrawal->id) }}" method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="mb-3 bg-light p-3 rounded-4">
                        ...
                        <p class="mb-1 text-muted small">Jumlah Pengajuan</p>
                        <h5 class="fw-bold text-danger m-0">Rp {{ number_format($withdrawal->amount, 0, ',', '.') }}</h5>
                    </div>
                    <div class="mb-3">
                        <label class="form-label text-muted small fw-bold">Alasan Penolakan</label>
                        <textarea name="rejection_reason" class="form-control" rows="3" placeholder="Masukkan alasan mengapa permintaan penarikan ini ditolak..." required></textarea>
                    </div>
                </div>
                <div class="modal-footer border-0 pt-0">
                    <button type="button" class="btn btn-light rounded-pill px-4" data-bs-dismiss="modal">Batal</button>
...
<div class="modal fade" id="viewProofModal{{ $withdrawal->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 16px; border: none;">
            <div class="modal-header border-0 pb-0">
                <h5 class="fw-bold">Struk Bukti Transfer Resmi</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4 text-center">
                ...
                <div class="bg-light p-3 rounded-4 text-start mt-2">
                    <div class="row small py-1">
                        <div class="col-4 text-muted">Dikirim Ke</div>
...
<div class="modal fade" id="viewRejectionModal{{ $withdrawal->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 16px; border: none;">
            <div class="modal-header border-0 pb-0">
                <h5 class="fw-bold text-danger">Detail Penolakan Penarikan</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4">
                <div class="bg-danger-subtle text-danger p-3 rounded-4 mb-3">
                    <div class="fw-bold mb-1 small">Alasan Ditolak:</div>
                ...
                <div class="bg-light p-3 rounded-4 text-start">
                    <div class="row small py-1">
                        <div class="col-4 text-muted">Tujuan Awal</div>
                        <div class="col-8 fw-bold">{{ strtoupper($withdrawal->bank_name) }} — {{ $withdrawal->account_number }} ({{ $withdrawal->account_holder }})</div>
                    </div>
                    <div class="row small py-1">
                        <div class="col-4 text-muted">Jumlah Awal</div>
                        <div class="col-8 fw-bold text-danger">Rp {{ number_format($withdrawal->amount, 0, ',', '.') }}</div>
                    </div>
                    <div class="row small py-1">
                        <div class="col-4 text-muted">Waktu Penolakan</div>
                        <div class="col-8 fw-bold">{{ $withdrawal->processed_at ? $withdrawal->processed_at->format('d M Y H:i') : '-' }}</div>
                    </div>
                </div>
            </div>
